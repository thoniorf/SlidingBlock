...
public class GamePanel extends JPanel {
	...
	private void setButtons() {
				...
				if (spostamenti.isEmpty()) {
					...
					for (Spostamenti s : spostamenti) {
						...
						frame.getGamePanel().paintImmediately(
								new Rectangle(MainFrame.sumX, MainFrame.sumY, MainFrame.sumX * 5, MainFrame.sumY * 5));
						try {
							Thread.sleep(1200);
		...
		this.help.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				if (spostamenti.isEmpty()) {
					...
					spostamenti.sort(null);
					int diffX = spostamenti.get(0).arrivoriga - spostamenti.get(0).partenzariga;
					int diffY = spostamenti.get(0).arrivocolonna - spostamenti.get(0).partenzacolonna;
					if (diffX < 0) {
						world.getMatrix().move(spostamenti.get(0).blocco, Direction.UP);
					} else if (diffX > 0) {
						world.getMatrix().move(spostamenti.get(0).blocco, Direction.DOWN);
					} else if (diffY < 0) {
						world.getMatrix().move(spostamenti.get(0).blocco, Direction.LEFT);
					} else if (diffY > 0) {
						world.getMatrix().move(spostamenti.get(0).blocco, Direction.RIGHT);
					}
					frame.getGamePanel().paintImmediately(
							new Rectangle(MainFrame.sumX, MainFrame.sumY, MainFrame.sumX * 5, MainFrame.sumY * 5));
					try {
		...
		this.help.setBounds((int) (this.getWidth() * 0.8), (int) (this.getHeight() * 0.35), help.getWidth(),
				help.getHeight());
		...
		this.comeToStart.setBounds((int) (this.getWidth() * 0.05), (int) (this.getHeight() * 0.65),
		...
		this.close.setBounds((int) (this.getWidth() * 0.05), (int) (this.getHeight() * 0.80), close.getWidth(),
				close.getHeight());
		this.add(dlv);
		this.add(help);
